...
嚴格規則（違反任一條視為品質不合格）：
1. 優先依照用戶原始訊息的意圖回答
2. Answer Contract 中有的事實可引用；Contract 為空時，依自身知識完成任務
3. 每個論點若 Contract 中有 citations，必須在敘述後標註「[來源 N]」（N 從 1 起）
4. caveats 列表不為空時才呈現，**空列表不得生出任何 caveats/注意事項段落**
5. 語氣依 response_mode：{response_mode}
6. **結尾不要加「歡迎隨時提出」「如需更多...請告知」等招攬語，直接結束**
    ...
    async def render(
        ...
        *,
        ...
        text = await self._render_text(
            contract=contract, skill=skill, response_mode=response_mode,
            feedback=feedback, user_input=user_input,
        )
